...
names(cols) = c("crumblr", "CLR", "LM (fraction)", "LM (log frac.)", "Binomial", "Beta-binomial", "NB", "Poisson", "scCoda")

cols = c(cols[1:4], c("LM (logit)" = "#07f500", "LM (asin)" = "#3acfb8"), cols[5:9])
...
fig1 = df_res %>%
		as_tibble %>%
		filter(assay == CT) %>%
		mutate(method = recode(method, poisson = "Poisson", nb = "NB", lm="LM (fraction)", lmlog = "LM (log frac.)", logit = "LM (logit)", asin = "LM (asin)", binomial="Binomial", betabinomial="Beta-binomial")) %>%
...
fig2 = df_res %>%
		as_tibble %>%
		filter(assay == CT) %>%
		mutate(method = recode(method, poisson = "Poisson", nb = "NB", lm="LM (fraction)", lmlog = "LM (log frac.)", binomial="Binomial", betabinomial="Beta-binomial")) %>%
