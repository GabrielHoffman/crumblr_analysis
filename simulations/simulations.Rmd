...
run_simulation = function( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, formula, methods, tau, use.sccoda=FALSE, mc.cores=20){
	...
	df_res = mclapply( 1:n_sims, function(i){
		...
		if( use.sccoda){
			res = sccoda_wrapper(counts, info, formula, python_path = "/hpc/users/hoffmg01/.cache/R/basilisk/1.8.0/0/bin/python")
		...
		if( ! is.numeric(df_methods$pValue)){
			browser()
		}
	...
	if( ! "p.value" %in% names(df_res)) browser()
...
fig1 = df_res2 %>%
	...
	# filter(Status != "none") %>%
		mutate(method = recode(method, poisson = "Poisson", nb = "NB", lm="Linear model", lmlog = "Linear model - log", binomial="Binomial", betabinomial="Beta-binomial")) %>%
		mutate(var.frac = format(var.batch / (var.batch + beta.true^2), digits=2)) %>%
...
# scCoda
# res = system.time({
# 		a = sccoda_wrapper(counts, info, formula, python_path = "/hpc/users/hoffmg01/.cache/R/basilisk/1.8.0/0/bin/python")
# })
