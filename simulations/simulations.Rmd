...
```{r plot.sims.mixed, cache=FALSE, fig.height=4, fig.width=10}
CT = "CT_1"
# AUPR
aupr.rand = unique(df_res$AUPR.rnd[!is.na(df_res$AUPR.rnd)])
fig1 = df_res %>%
...
df_res = lapply(c(5, 10, 20, 50), function(n_clusters){
	run_three_formulas( n_samples, n_clusters, mean_counts=1000*n_clusters, n_sims, beta.true, n_batches, var.batch, methods, tau, use.sccoda=FALSE, mc.cores=mc.cores) 
})
df_res = do.call(rbind, df_res)
```
