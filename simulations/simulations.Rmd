...
fig1 = df_res2 %>%
	...
	scale_y_continuous(expand=c(0, 0), limits=c(0, 1)) +
	 theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5), legend.position="none") +
	 geom_hline(yintercept=aupr.rand, linetype="dashed", color="black", size=3) +
	 xlab("Batch effect") + 
	 scale_fill_manual(values=cols) +
	 facet_wrap(~Status)
...
fig2 = df_res2 %>%
	...
	scale_y_continuous(expand=c(0, 0), limits=c(0, NA)) +
	 theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) +
	 geom_hline(yintercept=0.05, linetype="dashed", color="black", size=2) + 
	 xlab("Batch effect") + 
	 scale_fill_manual(values=cols)+
	 facet_wrap(~Status)
