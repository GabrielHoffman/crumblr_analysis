...
df_time_multiple = lapply( methods, function(method){
		...
		data.frame(Method = method, Time = as.numeric(res[3])/100)
})
...
fig = ggplot(df_time, aes(Method, pmax(2, Time) / 60, fill=Method)) +
...
fig = ggplot(df_time, aes(Method, pmax(1.5, Time), fill=Method)) +
	geom_bar(stat="identity") +
	theme_classic() +
	theme(aspect.ratio=1, legend.position="none") +
