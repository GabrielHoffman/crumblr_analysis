...
df_res2 = lapply(c(1e-2, 1e-1, .5), function(var.batch){  
...
fig2 = df_res2 %>%
	...
	mutate(method = recode(method, poisson = "Poisson", nb = "NB", lm="LM (fraction)", lmlog = "LM (log)", logit = "LM (logit)", asin = "LM (asin)", binomial="Binomial", betabinomial="Beta-binomial")) %>% 
	mutate(method = factor(method, sort(names(cols)))) %>%
	ggplot(aes(factor(var.batch), pmin(0.3, FPR), fill=method)) +
	geom_bar(stat="identity", position = "dodge") +
	theme_classic() +
	scale_y_continuous(expand=c(0, 0), limits=c(0, NA)) +
	 theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) +
...
df_time = rbind(df_time, data.frame(Method = "DESeq2", Time = as.numeric(res[3]) / 10))
...
df_time = rbind(df_time, data.frame(Method = "edgeR", Time = as.numeric(res[3]) / 10))
...
df_time = rbind(df_time, data.frame(Method = "Wilcoxon", Time = as.numeric(res[3]) / 10))
