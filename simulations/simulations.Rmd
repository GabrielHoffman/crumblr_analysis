...
# https://hoffmg01.dmz.hpc.mssm.edu/crumblr_analysis/simulations/simulations.html
...
run_simulation = function( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, formula, methods, tau, use.sccoda=FALSE, mc.cores=20){
	...
	info = data.frame(trait = sample(seq(2), n_samples, replace=TRUE), 
		Batch = factor(rep(sample.int(n_batches, n_samples/2, replace=TRUE), 2)))
	...
	df_res = mclapply( 1:n_sims, function(idx){
	    ...
	    dds <- nbinomWaldTest(dds)
			...
			design = model.matrix(nobars(formula), info)
			y <- estimateDisp(y, design)
			fit.egdeR <- glmQLFit(y, design)
			fit.egdeR <- glmLRT(fit.egdeR)
			tab.edgeR <- topTags(fit.egdeR, n=Inf,sort.by="none")

			# Wilcox
			frac = (counts+.25) / rowSums(counts)
	    res.wilcox = apply(frac, 2, function(x){
	    	trait = factor(info$trait)

	    	i = (trait == levels(trait)[1])
	      fitw = wilcox.test(x[i], x[!i])
	      fitw$p.value
	    })
			...
			df_p = df_p %>%
				left_join(data.frame(assay = rownames(tab.edgeR), 
					edgeR = tab.edgeR$table$PValue), 
				by="assay")

			df_p = df_p %>%
				left_join(data.frame(assay = names(res.wilcox), 
					Wilcoxon = res.wilcox), 
				by="assay")
		}
...
```{r plot.sims, cache=FALSE, fig.height=4, fig.width=10}
...
```{r plot.sims.scoda, cache=FALSE, fig.height=4, fig.width=10, eval=FALSE}
CT = "CT_2"
# AUPR
...
```{r plot.increasing, cache=FALSE, fig.width=10, fig.height=4}
CT = "CT_2"
...
```{r increase.tau}
df_res = lapply(c(1, 10, 20, 40), function(tau){
	run_three_formulas( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, methods, tau=tau, use.sccoda=FALSE, mc.cores=mc.cores) 
}) 
df_res = do.call(rbind, df_res)
```
...
```{r plot.increasing.tau, cache=FALSE, fig.width=10, fig.height=4}
CT = "CT_2"
# AUPR
aupr.rand = unique(df_res$AUPR.rnd[!is.na(df_res$AUPR.rnd)])
fig1 = df_res %>%
...
```{r increase.clusters} 
df_res = lapply(c(5, 15, 25, 40), function(n_clusters){
...
```{r plot.increase.clusters, cache=FALSE, fig.width=10, fig.height=4}
CT = "CT_2"
...
```{r plot.increase.samples, cache=FALSE, fig.width=10, fig.height=4}
CT = "CT_2"
# AUPR
aupr.rand = unique(df_res$AUPR.rnd[!is.na(df_res$AUPR.rnd)])
fig1 = df_res %>%
...
df_res2 = lapply(c(0, 1, 2, 4), function(var.batch){  
...
```{r sim2.plots, fig.height=4, fig.width=10, cache=FALSE, eval=TRUE}
aupr.rand = mean(df_res2$AUPR.rnd[!is.na(df_res$AUPR.rnd)])
df_res2 %>%
		as_tibble %>%
		filter(assay == CT) %>%
		mutate(method = recode(method, poisson = "Poisson", nb = "NB", lm="LM (fraction)", lmlog = "LM (log)", logit = "LM (logit)", asin = "LM (asin)", binomial="Binomial", betabinomial="Beta-binomial")) %>%
...
```{r plot.batch.effect, cache=FALSE, fig.width=10, fig.height=8}
CT = "CT_2"
# AUPR
aupr.rand = mean(df_res2$AUPR.rnd[!is.na(df_res2$AUPR.rnd)])
