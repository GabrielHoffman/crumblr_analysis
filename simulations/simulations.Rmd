...
```{r increase.clusters} 
df_res = lapply(c(10, 25, 50), function(n_clusters){
...
df_res2 = lapply(c(1e-2, 1e-1), function(var.frac){  
...
aupr.rand = mean(unique(df_res2$AUPR.rnd[!is.na(df_res$AUPR.rnd)]))
df_res2 %>%
		as_tibble %>%
		filter(assay == CT) %>%
		mutate(method = recode(method, poisson = "Poisson", nb = "NB", lm="LM (fraction)", lmlog = "LM (log)", logit = "LM (logit)", asin = "LM (asin)", binomial="Binomial", betabinomial="Beta-binomial")) %>%
		mutate(method = factor(method, sort(method))) %>%
