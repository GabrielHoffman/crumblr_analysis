...
conda activate /sc/arion/work/hoffmg01/condaEnv/r-reticulate
ml gcc/11.2.0 pandoc/2.6
export LD_LIBRARY_PATH=/sc/arion/work/hoffmg01/condaEnv/r-reticulate/lib/:$LD_LIBRARY_PATH
...
library(reticulate)
python_path = "/sc/arion/work/hoffmg01/condaEnv/r-reticulate/bin/python"
...
run_simulation = function( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, formula, methods, tau, use.sccoda=FALSE, mc.cores=20){
	...
	df_res = mclapply( 1:n_sims, function(idx){
		...
		if( idx %% 10 == 0) message("\r", idx, " / ", n_sims, '    ', appendLF=FALSE)
		...
		if( use.sccoda){
			res = sccoda_wrapper(counts, info, formula, python_path = "/sc/arion/work/hoffmg01/condaEnv/r-reticulate/bin/python")
...
df_res = run_three_formulas( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, methods, tau=tau, use.sccoda=TRUE, mc.cores=mc.cores, short=TRUE)  
```
...
```{r plot.sims.scoda, cache=FALSE, fig.height=4, fig.width=11}
cols = brewer.pal(9, "Set1")
cols[cols=="#FFFF33"] = "#FDDB27"
names(cols) = c("crumblr", "CLR", "Linear model", "Linear model - log", "Binomial", "Beta-binomial", "NB", "Poisson", "scCoda")
...
df_res2 = lapply(c(0, .01, .05, .1), function(var.frac){  

	var.batch = beta.true^2* var.frac / (1 - var.frac)

	run_three_formulas( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, methods, tau=tau, use.sccoda=FALSE, mc.cores=mc.cores, short=FALSE) 
}) 
...
# scCoda
res = system.time({
		a = sccoda_wrapper(counts, info, formula, python_path = "/sc/arion/work/hoffmg01/condaEnv/r-reticulate/bin/python")
})
