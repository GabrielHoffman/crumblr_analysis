...
df_res = run_three_formulas( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, methods, tau=3, use.sccoda=FALSE, mc.cores=36)  
```
...
```{r plot.increasing, cache=FALSE, fig.width=9, fig.height=5}
CT = "CT_1"
...
```{r plot.increasing.tau, cache=FALSE, fig.width=9, fig.height=5}
CT = "CT_1"
# AUPR
aupr.rand = unique(df_res$AUPR.rnd[!is.na(df_res$AUPR.rnd)])
...
```{r plot.increase.clusters, cache=FALSE, fig.width=9, fig.height=5}
CT = "CT_1"
...
df_time = rbind(df_time, data.frame(Method = "CLR", Time = as.numeric(res[3])))
...
df_time = rbind(df_time, data.frame(Method = "crumblr", Time = as.numeric(res[3])))
...
df_time_multiple = lapply( methods, function(method){
		...
		data.frame(Method = method, Time = as.numeric(res[3]))
})
df_time = rbind(df_time, do.call(rbind, df_time_multiple))
```

```{r plot.time, fig.width=4, fig.height=4, cache=FALSE}
ggplot(df_time, aes(Method, pmax(2, Time) / 60, fill=Method)) +
	geom_bar(stat="identity") +
	theme_classic() +
	theme(aspect.ratio=1, legend.position="none") +
	scale_y_continuous(limits=c(NA, NA), expand=c(0,0), name="Run time (minutes)") +
	coord_flip()
```
