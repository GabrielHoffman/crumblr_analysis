...
# library(reticulate)
# python_path = "/hpc/users/hoffmg01/.cache/R/basilisk/1.12.0/0/bin/python"
...
cap_ratio = function(W, max.ratio=10){
...
run_simulation = function( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, formula, methods, tau, use.sccoda=FALSE, mc.cores=20){
	...
	df_res = mclapply( 1:n_sims, function(i){
		...
		# crumblr
		cobj = crumblr(counts)
		cobj$weights = cap_ratio(cobj$weights, 5)
		fit = suppressMessages(dream(cobj, formula, X, BPPARAM=SerialParam()))
		fit = eBayes(fit)
		tab.crumblr = topTable(fit, coef="trait", sort.by="none")

		# # crumblr clr_2class
		# cobj2cl = crumblr(counts, method="clr_2class")
		# cobj2cl$weights = cap_ratio(cobj2cl$weights, 5)
...
df_res = run_three_formulas( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, methods, tau=tau, use.sccoda=FALSE, mc.cores=10, short=TRUE)  
```
...
df_res = lapply( c(100, 500,1000, 2000, 4000), function(mean_counts){
	run_three_formulas( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, methods, tau=tau, use.sccoda=FALSE, mc.cores=11) 
})
df_res = do.call(rbind, df_res)
```
...
df_res = lapply(c(1, 5, 10, 20), function(tau){
	run_three_formulas( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, methods, tau=tau, use.sccoda=FALSE, mc.cores=10) 
})
df_res = do.call(rbind, df_res)
```
...
df_res = lapply(c(5, 10, 20, 50), function(n_clusters){
	run_three_formulas( n_samples, n_clusters, mean_counts=1000*n_clusters, n_sims, beta.true, n_batches, var.batch, methods, tau, use.sccoda=FALSE, mc.cores=10) 
})
df_res = do.call(rbind, df_res)
```
...
df_res = lapply(c(20, 50, 100, 200), function(n_samples){
	run_three_formulas( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, methods, tau, use.sccoda=FALSE, mc.cores=10) 
})
df_res = do.call(rbind, df_res)
```
...
df_res2 = lapply(c(5000), function(mean_counts){  
	df = lapply(c( 0, .5, 1, 3), function(var.batch){  
		run_three_formulas( n_samples, n_clusters, mean_counts, n_sims, beta.true, n_batches, var.batch, methods, tau=2, use.sccoda=FALSE, mc.cores=10, short=FALSE)  
	})
	do.call(rbind, df)
})
