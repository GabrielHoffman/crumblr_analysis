...
rho_error = .5 # error correlation between responses
n_reps = 1e4
...
methods = c("FE", "tstat", "sidak", "fisher") 
...
```{r plot.FPR, fig.width=6, fig.height=5}
ggplot(df, aes(n_features, FPR, color=method)) +
  geom_line(size=1) + 
  geom_hline(yintercept=0.05, linetype="dashed") +
  theme_classic() +
...
```{r plot.lines, fig.width=6, fig.height=5}
ggplot(df, aes(n_positive, AUPR, color=method)) +
  geom_line(size=1) + 
  geom_hline(yintercept=0.1, linetype="dashed") +
  theme_classic() +
  theme(aspect.ratio=1) +
