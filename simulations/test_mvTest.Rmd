...
m.values = c(2, 8, 15, 25)
n.values = c(20, 50, 100, 200)
...
res = future_map_dfr( seq(n_reps), function(i){
  ...
  # Error variance
  error_var = matrix(runif(n.max*max(m.values), 1, 25), n.max, max(m.values))
  # error_var[] = 1

  Epsilon = sim_error(max(m.values), n.max, rho_error, error_var)

  beta.all = matrnorm(p,max(m.values))/10
...
```{r combine, fig.height=8, fig.width=16, cache=FALSE}
fig_AUPR
fig_FPR
```
