...
m.values = c(2, 8, 15) # responses , 12, 15, 20  5,
n.values = c(50, 100) # samples , 400 100,
  ...
  map_df( m.values, function(m){
    ...
    map_df( n.values, function(n){
      ...
      res = map_df(methods, function(method){
        mvTest(fit, vobj[,idx], rownames(fit), coef="V2", method=method, shrink.cov="FALSE")
      })
      res$i = i
      ...
      res = map_df(methods[3:4], function(method){
        mvTest(fit, vobj[,idx], rownames(fit), coef="V2", method=method, shrink.cov=TRUE)
      })
      res$i = i
      res$n_positive = n_positive
      res$shrink.cov = "Schafer"
      res$n = n
