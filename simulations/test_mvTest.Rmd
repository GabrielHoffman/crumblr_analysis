...
res = future_map_dfr( seq(n_reps), function(i){
  ...
  map_df( seq(2, m.max), function(m){
    ...
    # Error variance
    error_var = matrix(runif(n*m, 1, 100), n.max, m)
    # error_var[] = 1
    ...
    map_df( values, function(n_positive){
      ...
      maf_df( n.values, function(n){
        i = seq(n)
        # run dream
        suppressMessages(fit <- dream(vobj[,i], form, data=Xdf[i,]))
        fit = eBayes(fit)
        
        res = map_df(methods, function(method){
          mvTest(fit, vobj, rownames(fit), coef="V2", method, shrink.cov=FALSE)
        })
        res$i = i
        ...
        res2 = map_df(methods, function(method){
          mvTest(fit, vobj, rownames(fit), coef="V2", method, shrink.cov=TRUE)
        })
        res2$i = i
