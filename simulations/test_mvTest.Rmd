...
rho_error = .3 # error correlation between responses
n_reps = 1e3
...
res = future_map_dfr( seq(n_reps), function(i){
  ...
  # Error variance
  error_var = matrix(runif(n.max*max(m.values), 4, 6), n.max, max(m.values))
  # error_var[] = 1
...
```{r combine, fig.height=8, fig.width=20, cache=FALSE}
fig_AUPR
fig_FPR
```
