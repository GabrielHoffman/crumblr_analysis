...
res = future_map_df( seq(n_reps), function(i){
  ...
  map_df( values, function(n_positive){
    ...
    # run dream
    suppressMessages(fit <- dream(vobj$E, form, data=Xdf))
    fit = eBayes(fit)
    
    res = map_df(methods, function(method){
      mvTest(fit, vobj$E, rownames(fit), coef="V2", method)
    })
...
```{r histogram, fig.height=4, fig.width=10, cache=FALSE }
res %>%
  filter(i < n_reps/2) %>%
  group_by(method) %>%
