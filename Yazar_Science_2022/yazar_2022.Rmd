...
fig1 = plotTreeTestBeta(res) + ggtitle('Age') + theme(legend.position="none") 
...
i = 10
figList[[i]] + 
  geom_smooth(method=lm, formula = y ~ x + splines::bs(x, 3), color="green", se=FALSE) +
  geom_smooth(method="lm", formula = y ~ x + splines::bs(x, 3), weight=1, color="turquoise", se=FALSE)
```

<!----

form = ~ scale(age) + I(scale(age)^2) + (1|sex) + (1|pool_number) 
fitnl = dream(cobj, form, colData(pb))


tab = topTable(fitnl, coef="I(scale(age)^2)", number=Inf) %>%   
  select(logFC, AveExpr, t, P.Value, adj.P.Val)

CT = rownames(cobj$E)[i]
tab[CT,]
--->
