...
figList = lapply(rownames(cobj$E), function(CT){
  ...
  df %>% 
    ggplot(aes(age, CLR, color=se, weight=1/se^2)) +
      ...
      ggtitle(gsub(",", "\n", CT)) +
      scale_color_gradient2(low="grey", high="red", name = "SE") +
...
i = 10
figList[[i]] + 
  geom_smooth(method=lm, formula = y ~ x + splines::bs(x, 2), color="green", se=FALSE) +
  geom_smooth(method="lm", formula = y ~ x + splines::bs(x, 2), weight=1, color="turquoise", se=FALSE)
```
