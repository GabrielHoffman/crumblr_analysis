...
fig.tree = plotTreeTest(res1) + theme(legend.position="none")
...
```{r test.test2, fig.height=6, fig.width=12}
# combine plots
fig.tree = plotTreeTestBeta(res1) 
fig.logFC %>% insert_left(fig.tree)  %>% insert_right(fig.vp)
```

```{r treeTest2}
(crumblr::plotForest(res1, hide=TRUE) + theme(aspect.ratio=3)) %>%
  insert_left(plotTreeTestBeta(res1) )  %>% 
  insert_right(plotPercentBars( sortCols(vp), col=cols))
```
